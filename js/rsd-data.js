(function (global) {
  ...
  const DEFAULTS = {
    ...
    // 투자비 — 0.6승법 (규모지수법)
    //
    // 지수는 **설비 1기의 크기**에만 적용한다. 기수에까지 먹이면
    // "많이 지을수록 싸진다" 는 잘못된 결론이 나온다.
    // 기준 투자비는 전부 **가정값**이다 — 실제 견적으로 반드시 교체할 것.
    cost: {
      exponent: 0.6,        // 규모지수 — 화공·플랜트 견적의 표준값
      yard: { basisCapacity: 100000, baseCost: 0 },    // 1열 기준 (가정: 0원)
      shed: { basisCapacity: 250000, baseCost: 1000 }, // 1동 25만톤 기준 (가정)
      silo: { basisCapacity: 50000,  baseCost: 80 },   // 1기 5만톤 기준 (가정)
      source: '가정값 — 실제 견적으로 교체 필요'
    },
